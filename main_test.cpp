...
#include "part.h"
#include "distance.h"
#include "properties_bond.h"
#include "forces_bond_hybrid.h"
...
//            om[1] = 0.001;
        ...
        bondino = &vec_bond;
        bond_potyondy(bondino, vec_part[0], vec_part[1], dt);
        ...
        for (int j=0; j<n_part; j++)
        {
            ...
            if (j == IDS[0])
            {
                ...
                T_bond[0] = -(vec_bond.getTorque_st()[0] + vec_bond.getTorque_sr()[0] + vec_bond.getTorque_n()[0] + vec_bond.getTorque_tr()[0]);
                T_bond[1] = -(vec_bond.getTorque_st()[1] + vec_bond.getTorque_sr()[1] + vec_bond.getTorque_n()[1] + vec_bond.getTorque_tr()[1]);
                T_bond[2] = -(vec_bond.getTorque_st()[2] + vec_bond.getTorque_sr()[2] + vec_bond.getTorque_n()[2] + vec_bond.getTorque_tr()[2]);

//                T_bond[0] = -vec_bond.getTorque_s()[0];
//                T_bond[1] = -vec_bond.getTorque_s()[1];
//                T_bond[2] = -vec_bond.getTorque_s()[2];
            }
